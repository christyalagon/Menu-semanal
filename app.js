...
function load() {
  try {
    ...
    if (raw?.version === 1 && raw.stock && raw.done && Array.isArray(raw.purchases)) return {...raw,deductions:raw.deductions||{}};
  } catch { /* Empty or malformed local data. */ }
  return {version:1,stock:{},done:{},deductions:{},purchases:[]};
}
...
function shopCard(key,row) {
  ...
  return `<article class="shop-card"><div class="item-heading"><div><small>${category}</small><h3>${safe(name)}</h3></div><span class="status-dot ${row.toBuy?'missing':'ok'}" aria-hidden="true"></span></div><div class="item-stats"><div><span>En casa</span><b>${qty(key,row.available)}</b></div><div><span>Plan pendiente</span><b>${qty(key,row.required)}</b></div><div class="${need?'missing-text':'ok-text'}"><span>${label}</span><b>${qty(key,need)}</b></div></div><div class="coverage">${statusText(row)}</div><details><summary>Ver consumo por día</summary><div class="day-needs">${dayList||'<span>Sin comidas pendientes</span>'}</div></details><button class="buy-button" data-buy="${key}">+ Registrar compra</button></article>`;
}
function shopView() {
  const rows=forecast(week,state.stock,state.done);
...
function pantryView() {
  ...
  ${categories.map(cat=>`<section class="pantry-group"><h2>${cat}</h2><div class="pantry-items">${Object.keys(ITEMS).filter(k=>ITEMS[k][2]===cat).map(key=>`<button class="pantry-item" data-stock="${key}"><span>${safe(ITEMS[key][0])}<small>${rows[key].required?`${qty(key,rows[key].required)} pendiente esta semana`:'Sin consumo pendiente'}</small></span><strong class="${rows[key].available<0?'missing-text':''}">${qty(key,rows[key].available)}</strong><span class="pantry-arrow">›</span></button>`).join('')}</div></section>`).join('')}
...
function editStock(key) {
  const [name,unit]=ITEMS[key];openDialog(`<form id="stock-form" data-key="${key}"><button type="button" class="close" data-close aria-label="Cerrar">×</button><p class="eyebrow">AJUSTAR DESPENSA</p><h2>${safe(name)}</h2><p class="dialog-desc">Cantidad disponible ahora mismo. Úsalo para corregir el inventario inicial o cualquier diferencia.</p>${numberInput(state.stock[key]||0,unit)}<button class="primary-button" type="submit">Guardar cantidad</button></form>`);
}
function buy(key) {
...
dialog.addEventListener('submit',e=>{
  ...
  }else{state.stock[key]=amount;toast('Despensa actualizada');}
  closeDialog();save();
});
...
dialog.addEventListener('change',async e=>{
  ...
  try {
    ...
    state={version:1,stock,done,deductions,purchases};closeDialog();save();toast('Copia importada correctamente');
